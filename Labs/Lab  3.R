...
## read data
abalone <- read.csv("C:/Users/ardit/Downloads/abalone/abalone.data", header=FALSE)
...
## MODEL 1: weight features subset (3 features)
features1 <- c("whole_weight", "shucked_wieght", "shell_weight")
## MODEL 2: dimension and weight mixed subset (3 features)
features2 <- c("length", "diameter", "whole_weight")
...
# Model 1 - K = 10, weight features subset
...
cat("Model 1 Accuracy (weight subset):", round(m1_accuracy * 100, 2), "%\n")
...
# Model 2 - K = 10, mixed features subset
...
cat("Model 2 Accuracy (mixed subset):", round(m2_accuracy * 100, 2), "%\n")
...
if (m1_accuracy > m2_accuracy) {
  ...
  cat("Better Model: Model 1 (weight features subset)\n")
} else {
  better_features <- train2_norm
  better_test_features <- test2_norm
  better_feature_names <- features2
  cat("Better Model: Model 2 (mixed features subset)\n")
}
...
#USING K MEANS
#clustering testing from k to be 5-15
